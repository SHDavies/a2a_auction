...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create auction-e0fa
  fly apps create auction-e0fa-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app auction-e0fa
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app auction-e0fa-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'auction-e0fa-staging' does not match app name in config file 'auction-e0fa'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name auction-e0fa-db
  fly postgres attach --app auction-e0fa auction-e0fa-db

  fly postgres create --name auction-e0fa-staging-db
  fly postgres attach --app auction-e0fa-staging auction-e0fa-staging-db
  ```
